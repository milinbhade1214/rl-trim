...
class DDPG(object):
    def __init__(self, nb_states, nb_actions, args, text_writer):

        self.nb_states = nb_states
        self.nb_actions = nb_actions
        self.text_writer = text_writer
        
        print("N_b_states: ", self.nb_states)
        ...
        self.actor_scheduler = ExponentialLR(self.actor_optim, gamma=0.99)
        self.critic_scheduler = ExponentialLR(self.critic_optim, gamma=0.99)
    ...
    def update_policy(self):
        # Sample batch
        # print("Batch Sz: ", self.batch_size)
        ...
        policy_loss = policy_loss.mean()            #### Here needs to be changed
    ...
    # def select_action(self, s_t, episode):
    # action = to_numpy(self.actor(to_tensor(np.array(s_t).reshape(1, -1)))).squeeze(0)
    # if np.random.rand() < self.epsilon:  # With probability epsilon, take a random action
    #     action = np.random.uniform(self.lbound, self.rbound, self.nb_actions)
    # else:  # Otherwise, use the actor's action
    #     delta = self.init_delta * (self.delta_decay ** (episode - self.warmup))
    #     action = self.sample_from_truncated_normal_distribution(lower=self.lbound, upper=self.rbound, mu=action, sigma=delta)
    # action = np.clip(action, self.lbound, self.rbound)
    # return action

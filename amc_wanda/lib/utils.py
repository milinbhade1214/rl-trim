...
This is now correct
"""
def measure_layer_for_pruning(layer, input_ids, *args, **kwargs):
    def get_layer_type(layer):
        layer_str = str(layer)
        out = layer_str[:layer_str.find('(')].strip()
        # print(out)
        return out
    ...
    def get_layer_param(model):
        import operator
        import functools
        out = sum([functools.reduce(operator.mul, i.size(), 1) for i in model.parameters()])
        return out
    ...
    # ops_self_attention
    # ops_bert_attention
    elif type_name in ['LlamaSdpaAttention']:
        # Calculate FLOPs for BertAttention layer
        s, h = input_ids.size(1), input_ids.size(2)
        heads = layer.num_heads
        block_flops = dict(
            kqv = 3 * 2 * h * h,
            attn_scores = 2 * h * s,
            attn_softmax = SOFTMAX_FLOPS * s * heads, 
            attn_dropout = DROPOUT_FLOPS * s * heads,
            attn_scales = s * heads,
            attn_avg_weighted_values = 2 * s * h,
            attn_output = 2 * h * h,
        ...
        layer.flops = sum(block_flops.values())
        layer.params = 4 * h * h
    elif type_name in ['LlamaMLP']:
        s, h = input_ids.size(1), input_ids.size(2)
        ...
        block_flops = dict(
            ...
            up_proj = 2 * h * i,
            gate_up = h * i,
            up_act = ACTIVATION_FLOPS * i,
            down = 2 * i * h,
            down_dropout = DROPOUT_FLOPS * h,
            down_residual = h,
            down_layer_norm = LAYER_NORM_FLOPS,

...
class ChannelPruningEnv:
    ...
    def prune_kernel(self, op_idx, preserve_ratio, preserve_idx=None):
        ...
        if isinstance(op, LlamaSdpaAttention):
            query_weight = op.q_proj.weight.data.cpu().numpy()
            key_weight = op.k_proj.weight.data.cpu().numpy()
            value_weight = op.v_proj.weight.data.cpu().numpy()
            out_weight = op.o_proj.weight.data.cpu().numpy()
        elif isinstance(op, LlamaMLP):
            ## Get sum of each row 
            gate_weight = op.gate_proj.weight.data.cpu().numpy()
            up_weight = op.up_proj.weight.data.cpu().numpy()
            down_weight = op.down_proj.weight.data.cpu().numpy()
        ...
        if preserve_idx is None:
            ...
            if isinstance(op, LlamaSdpaAttention):
                ...
                # print("Using L2 Norm as criteria")
                q = torch.norm(torch.from_numpy(query_weight).view(32, -1), dim=1, p=2)
                v = torch.norm(torch.from_numpy(value_weight).view(32, -1), dim=1, p=2)
                k = torch.norm(torch.from_numpy(key_weight).view(32, -1), dim=1, p=2)
                o = torch.norm(torch.from_numpy(out_weight).view(32, -1), dim=1, p=2)
                importance = q + k + v + o
            ...
            # computer importance score for LlamaMLP layer
                ...
                ##########################
                g = torch.norm(torch.from_numpy(gate_weight), dim=1, p=2)
                u = torch.norm(torch.from_numpy(up_weight), dim=1, p=2)
                d = torch.norm(torch.from_numpy(down_weight), dim=1, p=2)
                importance = g + u + d 
                ##########################
        ...
        if isinstance(op, LlamaSdpaAttention):
            heads, index = find_pruneable_heads_and_indices(prune_idx, n_heads=32, head_size=128)
        ...
        if self.export_model:
            ...
            if isinstance(op, LlamaSdpaAttention):
                ...
                self.pruning_dict[op_idx] = preserve_idx.numpy().tolist()
